...
function generatePDFReport(reportData) {
  return new Promise((resolve, reject) => {
    ...
    if (partsRequest.length > 0) {
      // Start new page for parts if not enough room
      if (doc.y > 500) {
        doc.addPage();
        doc.y = 50;
      } else {
      ...
      // Parts header with green styling
      doc.fillColor('#22c55e')
         .fontSize(12)
         .font('Helvetica-Bold')
         .text('PARTS REQUEST', 50, doc.y);

      doc.y += 20;

      // Parts table header
      doc.fillColor('#f0fdf4')
         .rect(50, doc.y, pageWidth, 22)
         .fill();
      ...
      doc.text('Part Name / Description', 55, doc.y + 6);
      doc.text('Part #', 260, doc.y + 6);
      doc.text('Qty', 360, doc.y + 6);
      doc.text('Type', 400, doc.y + 6);
      doc.text('Status', 450, doc.y + 6);
      ...
      partsRequest.forEach((part, index) => {
        ...
        doc.text(part.partName || '-', 55, rowY + 5, { width: 200 });
        doc.text(part.partNumber || '-', 260, rowY + 5, { width: 95 });
        doc.text(String(part.quantity || 1), 360, rowY + 5);
        
        // Type indicator (Part or Labor)
        if (part.laborItem) {
          doc.fillColor('#3b82f6')
             .fontSize(9)
             .text('Labor', 400, rowY + 5);
        } else {
          doc.fillColor('#666666')
             .fontSize(9)
             .text('Part', 400, rowY + 5);
        }
        
        // Status indicator (In Stock / Order / N/A for labor)
        if (part.laborItem) {
          doc.fillColor('#666666')
             .fontSize(9)
             .text('-', 460, rowY + 5);
        } else if (part.inStock) {
          doc.fillColor('#22c55e')
             .fontSize(12)
             .text('✓', 460, rowY + 3);
        } else {
      ...
      // Parts summary
      doc.y += 10;
      const partItems = partsRequest.filter(p => !p.laborItem);
      const laborItems = partsRequest.filter(p => p.laborItem);
      const inStockCount = partItems.filter(p => p.inStock).length;
      const toOrderCount = partItems.filter(p => !p.inStock).length;

      doc.fillColor('#166534')
         .fontSize(10)
         .font('Helvetica-Bold')
         .text(`Summary: ${partItems.length} parts (${inStockCount} in stock, ${toOrderCount} to order) | ${laborItems.length} labor items`, 50, doc.y);
...
app.post('/api/submit-report', async (req, res) => {
  try {
    const { recipients, reportData } = req.body;
    
    // Support both old (email) and new (recipients) format
    const emailList = recipients || (req.body.email ? [req.body.email] : []);
    
    if (!emailList || emailList.length === 0 || !reportData) {
      return res.status(400).json({ success: false, error: 'Missing recipients or report data' });
    }
    
    console.log(`Generating professional PDF for ${emailList.length} recipient(s)...`);
    ...
    const totalSteps = reportData.totalSteps || 13;
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px; background: #f5f5f5;">
          ...
          ${reportData.partsRequest && reportData.partsRequest.length > 0 ? `
          <div style="margin-top: 20px; padding: 15px; background: #f0fdf4; border-radius: 8px; border: 1px solid #22c55e;">
            <h3 style="margin: 0 0 10px 0; color: #166534;">🔧 Parts & Labor Request (${reportData.partsRequest.length} items)</h3>
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr style="background: #dcfce7;">
                <th style="padding: 8px; text-align: left; border-bottom: 1px solid #22c55e;">Description</th>
                <th style="padding: 8px; text-align: left; border-bottom: 1px solid #22c55e;">Part #</th>
                <th style="padding: 8px; text-align: center; border-bottom: 1px solid #22c55e;">Qty</th>
                <th style="padding: 8px; text-align: center; border-bottom: 1px solid #22c55e;">Type</th>
                <th style="padding: 8px; text-align: center; border-bottom: 1px solid #22c55e;">Status</th>
              </tr>
              ${reportData.partsRequest.map(part => `
                <tr>
                  <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${part.partName || '-'}</td>
                  <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">${part.partNumber || '-'}</td>
                  <td style="padding: 8px; text-align: center; border-bottom: 1px solid #e5e7eb;">${part.quantity || 1}</td>
    ...
    if (failed > 0) {
      ...
      const failedEmails = emailList.filter((_, i) => results[i].status === 'rejected');
      console.warn('Failed recipients:', failedEmails);
    } else {
